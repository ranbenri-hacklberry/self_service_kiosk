-- ============================================================
-- 🔧 UPDATE SUBMIT_ORDER_V2 (SMART BUSINESS ID)
-- ============================================================
...
DECLARE
    ...
    v_business_id uuid;
BEGIN
    -- 1. RESOLVE BUSINESS ID (SMART LOGIC)
    -- Priority 1: Explicit Parameter
    -- Priority 2: Inferred from logged-in user (Secure)
    -- Priority 3: Hard fallback to Pilot Cafe
    
    v_business_id := p_business_id;
    
    IF v_business_id IS NULL THEN
        -- Try to get from current user (Workaround for SECURITY DEFINER: call our helper)
        -- Note: current_user_business_id() is also SECURITY DEFINER and checks auth.uid()
    ...
    IF edit_mode THEN
        ...
        IF jsonb_array_length(p_cancelled_items) > 0 THEN
            ...
            IF v_remaining_items = 0 THEN UPDATE orders SET order_status = 'cancelled' WHERE id = v_order_id; END IF;
        END IF;
    ...
    LOOP
        ...
        ELSE
            INSERT INTO order_items (
                ...
                course_stage, item_fired_at, business_id
            ) VALUES (
                v_order_id,
                (v_item->>'item_id')::int,
                (v_item->>'quantity')::int,
    ...
    -- INVENTORY DEDUCTION (NEW ORDERS ONLY)
