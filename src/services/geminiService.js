...
export const processInvoiceWithGemini = async (base64String, retryCount = 0) => {
    ...
    const prompt = `נתח את החשבונית המצורפת (תמונה או PDF) וחלץ את כל הפריטים למערך JSON.
שים לב לפרטים הקטנים: שמות פריטים בעברית כולל משקלים (למשל "תות שדה קפוא 1 ק"ג"), כמויות ויחידות מידה (יח', מארז, ק"ג).

עבור כל פריט, ספק את השדות הבאים:
- name: שם הפריט המלא בעברית (כולל משקל אם מופיע, למשל: "אננס קפוא 1 ק"ג")
- category: קטגוריה מתאימה (חלבי, ירקות, קפואים, פירות, יבשים, משקאות)
- unit: יחידת מידה בדיוק כפי שמופיע (יח', מארז, ק"ג, קרטון)
- quantity: הכמות המספרית בלבד (למשל אם כתוב "מארז 6" הכמות היא 6 והיחידה היא "מארז")
- price: מחיר ליחידה אחת כמספר בלבד (ללא סמל ₪)
- supplier_id: התאם את הספק: ${SUPPLIERS_LIST.map(s => `${s.name} (${s.id})`).join(', ')}. החזר רק את המספר.

החזר **רק** אובייקט JSON תקין בפורמט הבא:
{
  "supplier_detected": "שם הספק",
  "supplier_id": 5,
  "invoice_number": "מספר החשבונית/הזמנה",  
  "date": "YYYY-MM-DD",
  "total_amount": 0,
  "items": [
    { "name": "...", "category": "...", "unit": "...", "quantity": 0, "price": 0, "supplier_id": 5 }
  ]
}`;
