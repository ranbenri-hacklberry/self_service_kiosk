-- FORCE UPDATE KDS FUNCTION
-- Ensure no old version lingers
...
BEGIN
    ...
    -- 1. Try identifying by direct auth_user_id link
    SELECT business_id INTO v_business_id
    FROM employees
    WHERE auth_user_id = v_user_id
    LIMIT 1;

    -- 2. Fallback: Identify by Phone (for Demo users)
    ...
    -- 3. STRICT SECURITY: If no business identified, return NOTHING.
    IF v_business_id IS NULL THEN
        RETURN;
    END IF;

    -- 4. Return Orders for the identified business ONLY
    RETURN QUERY
    SELECT 
        o.id,
        o.order_number,
    ...
    WHERE 
        o.business_id = v_business_id  -- <--- THE FILTER
