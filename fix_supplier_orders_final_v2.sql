-- Fix RLS policies for Supplier Orders and Items (UUID Corrected)
...
-- Note: 'business_id' in JWT is text, but column is UUID. We explicitly cast BOTH to uuid for safety.
...
-- Ideally strict RLS would check the parent order's business_id via join, but this is acceptable for internal tools.
