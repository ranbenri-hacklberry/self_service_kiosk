...
ADD COLUMN IF NOT EXISTS course_stage INTEGER DEFAULT 1,
ADD COLUMN IF NOT EXISTS item_fired_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- 2. Update submit_order_v2 RPC to handle course_stage and item_fired_at
...
BEGIN
    ...
    LOOP
        ...
        v_item_fired_at := NOW(); -- Default to now
        ...
        IF v_order_item_id_str IS NOT NULL AND v_order_item_id_str != 'null' AND v_order_item_id_str != '' THEN
             ...
             SET 
                ...
                -- If we are un-holding (editing back to stage 1), we might want to reset status?
                -- For now, let's assume status logic is handled by frontend passing 'item_status' or we default it.
                -- If we switch from stage 2 to 1, we should probably set status to 'in_progress' if it was 'held'.
                item_status = CASE 
