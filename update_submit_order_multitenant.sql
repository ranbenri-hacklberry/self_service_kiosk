-- ============================================================
-- 🔧 UPDATE SUBMIT_ORDER_V2 FOR MULTI-TENANCY (FIXED)
-- ============================================================

-- 1. DROP ALL EXISTING VERSIONS OF THE FUNCTION TO AVOID CONFLICTS
DO $$ 
DECLARE 
    r RECORD;
BEGIN 
    FOR r IN (
        SELECT pg_proc.oid, pg_get_function_identity_arguments(pg_proc.oid) as args 
        FROM pg_proc 
        JOIN pg_namespace n ON pg_proc.pronamespace = n.oid 
        WHERE proname = 'submit_order_v2' AND n.nspname = 'public'
    ) 
    LOOP 
        EXECUTE 'DROP FUNCTION IF EXISTS public.submit_order_v2(' || r.args || ') CASCADE'; 
    END LOOP; 
END $$;
...
CREATE OR REPLACE FUNCTION public.submit_order_v2(
    ...
    p_business_id uuid DEFAULT NULL -- NEW PARAMETER, Default NULL to allow logic to catch it
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Priority: 
    -- 1) Passed Parameter (p_business_id)
    -- 2) Fallback to 'Pilot Cafe' for legacy clients/testing (1111...)
    IF p_business_id IS NOT NULL THEN
        v_business_id := p_business_id;
    ELSE
        -- Fallback default for safety (Pilot Cafe)
    ...
    IF edit_mode THEN
        ...
        -- (Logic continues for Edit Mode...)
        -- Note: We DON'T update business_id on edit, it should stay with original owner.
    ...
    ELSE
        ...
        INSERT INTO orders (
            ...
            is_refund, refund_amount, business_id -- Added business_id
        ) VALUES (
    ...
    LOOP
        ...
        -- Set fired_at based on stage
        IF v_course_stage = 2 THEN
            v_item_fired_at := NULL;
        ELSE
            v_item_fired_at := NOW();
        END IF;
        ...
        ELSE
            ...
            INSERT INTO order_items (
                ...
                course_stage, item_fired_at, business_id -- Added business_id
            ) VALUES (
                v_order_id,
                (v_item->>'item_id')::int,
                (v_item->>'quantity')::int,
